...
import { useCategoriesCascader } from './useCategoriesCascader.js'
...
// 從 useCategoriesCascader 組合式函數中獲取狀態和方法
const { loading, cascaderProps: baseCascaderProps, getPath } = useCategoriesCascader(props.leafOnly)
...
const cascaderProps = computed(() => ({
  ...baseCascaderProps.value,  // 從 useCategoriesCascader 獲取的基礎屬性
...
const handleChange = async (value: number[]) => {
  // 提取路徑的最後一個元素作為選中的 ID
  const selectedId = value && value.length > 0 ? value[value.length - 1] : null
