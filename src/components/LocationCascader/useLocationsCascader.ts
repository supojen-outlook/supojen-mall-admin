 ...
 */
export function useLocationsCascader(leafOnly: boolean = false) {
  // 加載狀態
  ...
  const cascaderProps = ref<CascaderProps>({
    ...
    async lazyLoad(node, resolve: (nodes: CascaderOption[]) => void) {
      try {
        ...
        if (level === 0) {
          // 加載根節點（level 1 的位置，即區域）
          const rootLocations = await getLocations({
            level: 1,
            size: 1000 // 獲取所有 level 1 位置
          });
          nodes = rootLocations.list.map(location => ({
            value: location.id,
            label: location.name,
            leaf: location.level === 2, // 層級為2的是葉子節點（儲位）
            ...location // Spread the rest of the properties
          } as CascaderOption));
        } else {
          // 加載子節點
          const children = await getLocations({
            parentId: data.value as number,
            size: 1000 // 獲取所有子節點
          });
          nodes = children.list.map(location => ({
            value: location.id,
            label: location.name,
            leaf: location.level === 2, // 層級為2的是葉子節點（儲位）
            ...location // Spread the rest of the properties
  ...
  /**
   * 根據位置ID獲取從根節點到該位置的完整路徑
   * 
   * @param id - 要查詢的位置ID
   * @returns 返回一個 Promise，解析為包含從根節點到該位置的ID路徑陣列
   *          如果獲取路徑失敗，則返回包含原始ID的陣列作為回退
   * @example
   * // 返回範例: [1, 5, 12] 表示根節點(1) -> 一級位置(5) -> 當前位置(12)
   * const path = await getPath(12);
   */
  async function getPath(id: number): Promise<number[]> {
    try {
      const response = await getLocationPath(id);
      return response || [id];
    } catch (error) {
      console.error('獲取位置路徑失敗:', error);
   ...
   * 批量獲取多個位置ID的完整路徑
   * 
   * @param ids - 要查詢的位置ID陣列
   * @returns 返回一個 Promise，解析為包含所有路徑的二維陣列
   * @example
   * // 返回範例: [[1, 5, 12], [1, 6, 15]] 表示兩個位置的完整路徑
  ...
  async function getPaths(ids: number[]): Promise<number[][]> {
    try {
      ...
      console.error('批量獲取位置路徑失敗:', error);
      // 如果出錯，返回單個 ID 作為回退
  ...
  async function getAllChildIds(parentId: number): Promise<number[]> {
    try {
      ...
      for (const child of children.list) {
        ...
        // 如果這個子節點還有子節點，遞歸獲取
        if (child.level < 2) { // 層級小於2的還可能有子節點
